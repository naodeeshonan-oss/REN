// /api/interrogate.js  — 画像をタグ文字列に要約して返す
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const r = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "user",
          content: [
            { type: "text", text: "Describe this image as concise, comma-separated tags for anime-style generation. No sentences. 30~60 tokens." },
    ...
    if (!r.ok) return res.status(r.status).json({ error: data?.error?.message || "interrogate失敗" });
